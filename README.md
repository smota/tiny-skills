...
| software-engineering | [agent-refine](skills/software-engineering/agent-refine/) | Distill session lessons and consolidate scoped agent-instruction layers with optional recursive and advisory-guidance review | Project read access and visible session context; write access only for confirmed changes | `npx skills add smota/tiny-skills --skill agent-refine` |
...
| software-engineering | [techlead-planner](skills/software-engineering/techlead-planner/) | Turn a goal into a reviewed plan before implementation: gate questions, vertical slices, ordering, architecture placement, and a non-functional pass | Project read access; optional read-only commands for evidence | `npx skills add smota/tiny-skills --skill techlead-planner` |
| software-engineering | [cross-review](skills/software-engineering/cross-review/) | Run an independent, read-only adversarial review of a change in a different harness or model than the author's, and report findings with provenance | Git and repository read access; a second reviewer CLI (Claude Code, Codex, or pi) signed in; consent to send code to its provider | `npx skills add smota/tiny-skills --skill cross-review` |
