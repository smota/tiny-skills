...
| Tiered orchestration chosen; the catalog row for a needed model is `unverified` or over 90 days old | Checks the provider's own pricing page before locking a budget and reports the difference | Budgets from the catalog price as if it were current |
