...
8. **Set the quality gates.** State done for each slice as checks that can run, name the review points, give a rollback for every irreversible step (or the decision that none exists), and list the decisions that need an ADR. **Complete when:** every done condition is checkable and every irreversible step has a rollback or an explicit no-rollback decision.
